...
class ImageFilterService {
  ...
  static img.Image _applyUrbanFade(img.Image image) {
    // Desaturated soft tone
    image = img.adjustColor(image, saturation: 0.3, brightness: 1.05);
    image = img.contrast(image, contrast: 90);
    return image;
  }

  static img.Image _applySoftMatte(img.Image image) {
    // Low contrast with soft brightness
    image = img.contrast(image, contrast: 70);
    image = img.adjustColor(image, saturation: 0.6, brightness: 1.15);
    // Add a subtle blur for matte effect
    image = img.gaussianBlur(image, radius: 1);
    return image;
  }

  static img.Image _applyVintageSilver(img.Image image) {
    // Grayscale with subtle sepia
    image = img.grayscale(image);
  ...
  static img.Image _applyCoolChrome(img.Image image) {
    // Bluish gray with enhanced highlights
    image = img.adjustColor(
      image,
      saturation: 0.4,
      hue: 0.1, // Slight blue shift
    );
    ...
    for (int i = 0; i < pixels.length; i += 4) {
      ...
      if (luminance > 128) {
        // Enhance highlights with blue tint
        pixels[i] = (r * 0.9).clamp(0, 255).round(); // Reduce red
        pixels[i + 1] = (g * 0.95).clamp(0, 255).round(); // Slight reduce green
        pixels[i + 2] = (b * 1.1).clamp(0, 255).round(); // Enhance blue
      }
    }
  ...
  static img.Image _applyDramaticBlack(img.Image image) {
    // High contrast black and white
    image = img.grayscale(image);
    image = img.contrast(image, contrast: 150);
    image = img.adjustColor(image, brightness: 0.95, gamma: 1.3);
    return image;
  }
  ...
  static img.Image _applyEtherealGlow(img.Image image) {
    // Soft dreamy highlights
    image = img.adjustColor(image, saturation: 0.2, brightness: 1.2);
    image = img.gaussianBlur(image, radius: 2);
    image = img.contrast(image, contrast: 80);

    // Add glow effect to highlights
    final pixels = image.getBytes();
    for (int i = 0; i < pixels.length; i += 4) {
      final r = pixels[i];
      final g = pixels[i + 1];
      ...
      if (luminance > 150) {
        // Add warm glow to highlights
        pixels[i] = (r * 1.1).clamp(0, 255).round();
        pixels[i + 1] = (g * 1.05).clamp(0, 255).round();
        pixels[i + 2] = (b * 0.9).clamp(0, 255).round();
      }
    }
  ...
  static img.Image _applyIndustrialGrit(img.Image image) {
    // Raw urban texture
    image = img.adjustColor(image, saturation: 0.4, brightness: 0.9);
    image = img.contrast(image, contrast: 140);

    // Add noise for gritty texture
    final pixels = image.getBytes();
    for (int i = 0; i < pixels.length; i += 4) {
      final noise = (i % 7 - 3) * 2; // Simple noise pattern
      pixels[i] = (pixels[i] + noise).clamp(0, 255);
      pixels[i + 1] = (pixels[i + 1] + noise).clamp(0, 255);
      pixels[i + 2] = (pixels[i + 2] + noise).clamp(0, 255);
    }

    return image;
  }

  static img.Image _applyDreamyFog(img.Image image) {
    // Misty atmospheric effect
    image = img.adjustColor(image, saturation: 0.3, brightness: 1.15);
    image = img.gaussianBlur(image, radius: 3);
    image = img.contrast(image, contrast: 60);

    // Add fog overlay
    final pixels = image.getBytes();
    for (int i = 0; i < pixels.length; i += 4) {
      pixels[i] = (pixels[i] * 0.8 + 200 * 0.2).clamp(0, 255).round();
      pixels[i + 1] = (pixels[i + 1] * 0.8 + 210 * 0.2).clamp(0, 255).round();
      pixels[i + 2] = (pixels[i + 2] * 0.8 + 220 * 0.2).clamp(0, 255).round();
    }

    return image;
  }

  static img.Image _applyRetroContrast(img.Image image) {
    // Vintage high-contrast look
    image = img.adjustColor(image, saturation: 0.7, brightness: 1.0);
    image = img.contrast(image, contrast: 135);

    // Add vintage color cast
    final pixels = image.getBytes();
    for (int i = 0; i < pixels.length; i += 4) {
      pixels[i] = (pixels[i] * 1.05).clamp(0, 255).round(); // Slightly warm
      pixels[i + 1] = (pixels[i + 1] * 0.98).clamp(0, 255).round();
      pixels[i + 2] = (pixels[i + 2] * 0.92)
          .clamp(0, 255)
          .round(); // Reduce blue
    }
  ...
  static img.Image _applyCyberpunkEdge(img.Image image) {
    // Futuristic neon with high contrast - Gen Z favorite
    image = img.adjustColor(
      image,
      saturation: 1.6,
      brightness: 0.9,
    ...
    for (int i = 0; i < pixels.length; i += 4) {
      ...
      if (intensity > 150) {
        ...
        pixels[i] = (255 * 0.8).round(); // Neon pink/red
        pixels[i + 1] = (255 * 0.2).round();
        pixels[i + 2] = (255 * 1.0).round(); // Electric blue
      } else if (intensity > 80) {
        // Mid tones = electric cyan
        pixels[i] = (r * 0.3).clamp(0, 255).round();
        pixels[i + 1] = (g * 1.4).clamp(0, 255).round();
        pixels[i + 2] = (b * 1.5).clamp(0, 255).round();
      } else {
        // Deep shadows = dark purple
        pixels[i] = (r * 0.8).clamp(0, 255).round();
        pixels[i + 1] = (g * 0.3).clamp(0, 255).round();
  ...
  static img.Image _applyBohoChic(img.Image image) {
    // Bohemian vibes with muted earth tones - trending lifestyle aesthetic
    image = img.adjustColor(
      image,
      saturation: 0.75,
      brightness: 1.02,
    ...
    for (int i = 0; i < pixels.length; i += 4) {
      ...
      pixels[i] = (r * 1.08 + 15).clamp(0, 255).round(); // Warm terracotta
      pixels[i + 1] = (g * 1.05 + 10).clamp(0, 255).round(); // Sage green
      pixels[i + 2] = (b * 0.9 + 5).clamp(0, 255).round(); // Muted blue
    }
